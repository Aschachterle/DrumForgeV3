...
(async () => {
  try {
    ...
    const payload = {
      ...
      commandLine: [
        '$(engine.path)\\FusionAutomation.exe',
        '"$(args[inputFile].path)"',
        '"$(args[outputFile].path)"',
        '--script', '"$(appbundles[DrumModifier].path)\\Contents\\DrumModifier.ts"',
        '--params', '"$(args[paramsFile].path)"'
      ],
      parameters: {
        inputFile: {
          verb: 'get',
      ...
      description: 'Modifies drum parameters in Fusion 360 F3D files (JavaScript)'
    };
    
    console.log('Creating Activity version 12 (Script invocation with Contents path)...');
