...
export const buildPreviewSvg = (parameters) => {
  ...
  const sideY = padding + topRadius * 2 + 55;
  ...
  for (let i = 0; i < segments; i++) {
    ...
    topCutLines += `<line x1="${outerX}" y1="${outerY}" x2="${innerX}" y2="${innerY}" stroke="#333" stroke-width="1" stroke-dasharray="2,3"/>`;
    ...
    topCutLines += `<path d="M${lapStartX},${lapStartY} A${midRadius},${midRadius} 0 0,0 ${lapEndX},${lapEndY}" fill="none" stroke="#999" stroke-width="0.75" stroke-dasharray="1,2"/>`;
  }
...
<svg xmlns="http://www.w3.org/2000/svg" width="${svgWidth}" height="${svgHeight}" viewBox="0 0 ${svgWidth} ${svgHeight}">
  ...
  <!-- TOP VIEW -->
  <text x="${topCx}" y="18" font-family="Arial, sans-serif" font-size="11" font-weight="bold" text-anchor="middle">TOP VIEW</text>
  ...
  <text x="${topCx}" y="${topCy - 8}" font-family="Arial, sans-serif" font-size="7" text-anchor="middle" font-weight="bold">${segments} seg</text>
  <text x="${topCx}" y="${topCy}" font-family="Arial, sans-serif" font-size="7" text-anchor="middle">${lapPct}% lap</text>
  <text x="${topCx}" y="${topCy + 8}" font-family="Arial, sans-serif" font-size="7" text-anchor="middle">${thickMm}mm shell</text>
  
  <!-- Diameter dimension -->
  <line x1="${topCx - topRadius}" y1="${topCy + topRadius + 15}" x2="${topCx + topRadius}" y2="${topCy + topRadius + 15}" stroke="#000" stroke-width="1" marker-start="url(#arrow)" marker-end="url(#arrow)"/>
  <text x="${topCx}" y="${topCy + topRadius + 28}" font-family="Arial, sans-serif" font-size="10" text-anchor="middle">⌀${diam}"</text>
  
  <!-- SIDE VIEW -->
  <text x="${sideX + sideWidth / 2}" y="${sideY - 10}" font-family="Arial, sans-serif" font-size="11" font-weight="bold" text-anchor="middle">SIDE VIEW</text>
  ...
  <!-- Left wall - outer -->
  <line x1="${sideX}" y1="${sideY}" x2="${sideX}" y2="${sideY + sideHeight}" stroke="#000" stroke-width="1"/>
  <!-- Left wall - inner -->
  <line x1="${sideX + visualThickPx}" y1="${sideY}" x2="${sideX + visualThickPx}" y2="${sideY + sideHeight}" stroke="#000" stroke-width="1"/>
  <!-- Right wall - outer -->
  ...
  <line x1="${sideX}" y1="${sideY + sideHeight + 8}" x2="${sideX + visualThickPx}" y2="${sideY + sideHeight + 8}" stroke="#666" stroke-width="1"/>
  <!-- Tick marks -->
  <line x1="${sideX}" y1="${sideY + sideHeight + 5}" x2="${sideX}" y2="${sideY + sideHeight + 11}" stroke="#666" stroke-width="1"/>
  <line x1="${sideX + visualThickPx}" y1="${sideY + sideHeight + 5}" x2="${sideX + visualThickPx}" y2="${sideY + sideHeight + 11}" stroke="#666" stroke-width="1"/>
  <!-- Measurement -->
  <text x="${(sideX + sideX + visualThickPx) / 2}" y="${sideY + sideHeight + 18}" font-family="Arial, sans-serif" font-size="7" text-anchor="middle">${thickMm}mm shell</text>
  ...
  <line x1="${sideX + sideWidth - 15 - lugR}" y1="${sideY + lugBottomPx}" x2="${sideX + sideWidth - 15 + lugR}" y2="${sideY + lugBottomPx}" stroke="#666" stroke-width="1" marker-start="url(#smallArrow)" marker-end="url(#smallArrow)"/>
  <text x="${sideX + sideWidth - 15}" y="${sideY + lugBottomPx + lugR + 10}" font-family="Arial, sans-serif" font-size="8" text-anchor="middle">⌀${lugDiam}"</text>
  
  <!-- Height dimension -->
  <line x1="${sideX + sideWidth + 12}" y1="${sideY}" x2="${sideX + sideWidth + 12}" y2="${sideY + sideHeight}" stroke="#000" stroke-width="1" marker-start="url(#arrow)" marker-end="url(#arrow)"/>
  ...
  <line x1="${sideX - 18}" y1="${sideY}" x2="${sideX - 18}" y2="${sideY + lugTopPx}" stroke="#666" stroke-width="1"/>
  <line x1="${sideX - 22}" y1="${sideY}" x2="${sideX - 14}" y2="${sideY}" stroke="#666" stroke-width="1"/>
  <line x1="${sideX - 25}" y1="${sideY + lugTopPx}" x2="${sideX - 11}" y2="${sideY + lugTopPx}" stroke="#666" stroke-width="1.5"/>
  ...
  <line x1="${sideX - 18}" y1="${sideY + lugTopPx}" x2="${sideX - 18}" y2="${sideY + lugBottomPx}" stroke="#666" stroke-width="1"/>
  <line x1="${sideX - 22}" y1="${sideY + lugBottomPx}" x2="${sideX - 14}" y2="${sideY + lugBottomPx}" stroke="#666" stroke-width="1"/>
